...
G4VPhysicalVolume* DetectorConstruction::ConstructDetector()
{
  ...
  experimentalHall_box
    = new G4Box("expHall_box",0.8*m,0.8*m,0.8*m);
...
//paddle 
  G4Box* paddle_box = new G4Box("paddle_box",20*cm,60*cm,
			    60*cm);
 ...
 //----------------------- head_Con
  ...
  G4double  pRmax1 = sqrt(1800)*0.5*mm;//sqrt(3604)*0.5*mm
  G4double  pRmin2 = 0 *mm;
  G4double  pRmax2 =15*mm;// 26*mm;
  G4double  pDConz =20*mm;
  G4Cons* head_Con=new G4Cons("head_Con",pRmin1,pRmax1,pRmin2,pRmax2,pDConz,0.0,360.0*deg);

 //----------------------- head_Trd

  G4double  dx1 = 15*mm;
  G4double  dx2 = 15*mm;//26*mm;
  G4double  dy1 = 15*mm;//26*mm;
  G4double  dy2 = 15*mm;//26*mm;
...
//------------------------------ end
 ...
 //---------------------------- head+end
  ...
  G4double mPos_z =pDConz+pDcz-0.001*mm;
...
//----------------------------scintillator_middle
  G4double  scintz = 380*mm;//412.5*mm;
  G4Box* scint0 = new G4Box("scint0",dx1,scintz,scintz);
  G4double dyz=scintz*sqrt(2.0)-dy1;
  G4Box* scint1 = new G4Box("scint1",dx1,dyz,dyz);

  G4RotationMatrix* s0Rot = new G4RotationMatrix;
  s0Rot->rotateX(45.*deg);
  G4ThreeVector s0Trans(0.,0.,0.);
  G4IntersectionSolid* scint_middle = new G4IntersectionSolid("scint_middle", scint0, scint1, s0Rot,s0Trans);

//--------------------------scintillator_middle+guide=scint

  G4double s1Pos_x = 0. *mm;
  G4double s1Pos_y = (dyz+pDConz)/sqrt(2.0)-0.01*mm;
  G4double s1Pos_z = (dyz+pDConz)/sqrt(2.0)-0.01*mm;

  G4RotationMatrix* s1Rot = new G4RotationMatrix;
  s1Rot->rotateX(-45.*deg);
  G4ThreeVector s1Trans(s1Pos_x,s1Pos_y,s1Pos_z);
  G4UnionSolid* scint_1= new G4UnionSolid("scint_1", scint_middle, guide, s1Rot, s1Trans);

  s1Rot->rotateX(180.*deg);
  s1Trans=G4ThreeVector(s1Pos_x,-s1Pos_y,-s1Pos_z);
  G4UnionSolid* scint_2= new G4UnionSolid("scint_2", scint_1, guide, s1Rot, s1Trans);

  s1Rot->rotateX(90.*deg);
  s1Trans=G4ThreeVector(s1Pos_x,s1Pos_y,-s1Pos_z);
  G4UnionSolid* scint_3= new G4UnionSolid("scint_3", scint_2, guide, s1Rot, s1Trans);
...
//--------------------------scintillator_up+guide=scint
  ...
  s1Rot->rotateX(180.*deg);
  s1Trans=G4ThreeVector(s1Pos_x,-s1Pos_y,s1Pos_z);
  G4UnionSolid* scint_union= new G4UnionSolid("scint_union", scint_3, guide, s1Rot, s1Trans);

  G4LogicalVolume* scint_log = new G4LogicalVolume(scint_union,Scintillator,
				    "scint_log",0,0,0);
  G4PVPlacement* scint_phys = new G4PVPlacement(0,G4ThreeVector(),scint_log,"scintp",
				   paddle_log,false,0); 
  //-----------------------  guide

  G4double guidePos_x = 0. *mm;
  G4double guidePos_y = s1Pos_y;
  G4double guidePos_z = s1Pos_z;
  ...
  G4PVPlacement* guide1_phys= new G4PVPlacement(G4Transform3D(gRot,gpos),
                                                 guide_log,"guide",scint_log,false,0);
  gRot.rotateX(180.*deg);
  gpos=G4ThreeVector(guidePos_x,-guidePos_y,-guidePos_z);
  G4PVPlacement* guide3_phys = new G4PVPlacement(G4Transform3D(gRot,gpos),
                                  guide_log,"guide",scint_log,false,2);
  gRot.rotateX(90.*deg);
  gpos=G4ThreeVector(guidePos_x,guidePos_y,-guidePos_z);
  G4PVPlacement* guide2_phys = new G4PVPlacement(G4Transform3D(gRot,gpos),
                                  guide_log,"guide",scint_log,false,3);
  gRot.rotateX(180.*deg);
  gpos=G4ThreeVector(guidePos_x,-guidePos_y,guidePos_z);
  G4PVPlacement* guide4_phys = new G4PVPlacement(G4Transform3D(gRot,gpos),
                                  guide_log,"guide",scint_log,false,1);
 /////////////////////////////////////////////////////////////
   //****************** Build PMTs
    G4double innerRadius_pmt = 0.*cm;
    G4double outerRadius_pmt =14*mm;// 23*mm;
    ...
    G4ThreeVector  phpos=G4ThreeVector(0.*mm,0.0*mm,pDConz+pDcz-0.001*mm);
    ...
    //guide_log->SetVisAttributes(pmt_va);
...
//--------------------------------scint guide
  ...
  new G4LogicalBorderSurface("GuideSurface1",guide1_phys,scint_phys,GuideSurface); 
  new G4LogicalBorderSurface("GuideSurface2",guide2_phys,scint_phys,GuideSurface); 
  new G4LogicalBorderSurface("GuideSurface3",guide3_phys,scint_phys,GuideSurface); 
  new G4LogicalBorderSurface("GuideSurface4",guide4_phys,scint_phys,GuideSurface); 
  ...
  new G4LogicalBorderSurface("GDSurface1",guide1_phys,pmt_phys,OppmtSurfaceGD);
  new G4LogicalBorderSurface("GDSurface2",guide2_phys,pmt_phys,OppmtSurfaceGD);
  new G4LogicalBorderSurface("GDSurface3",guide3_phys,pmt_phys,OppmtSurfaceGD);
  new G4LogicalBorderSurface("GDSurface4",guide4_phys,pmt_phys,OppmtSurfaceGD);
