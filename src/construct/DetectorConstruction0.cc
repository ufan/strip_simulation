...
G4VPhysicalVolume* DetectorConstruction::ConstructDetector()
{
  ...
  experimentalHall_box
    = new G4Box("expHall_box",0.4*m,0.4*m,0.4*m);
...
//paddle 
 ...
 //----------------------- head_Con
  ...
  G4double  pRmax1 = 100*sqrt(2.0)*mm;
  G4double  pRmin2 = 0 *mm;
  G4double  pRmax2 = 25*mm;
  G4double  pDConz =75*mm-25*mm-25*mm-23*mm;
 ...
 //----------------------- head_Trd
  ...
  G4double  dy1 = 100*mm;
  G4double  dy2 = 175*mm;
  G4double  pDTrdz = pDConz+0.01*mm;
  G4Trd* head_Trd=new G4Trd("head_Trd",dx1,dx2,dy1,dy2,pDTrdz);
//----------------------- head_1

  G4double Pos_x1 = 200.*mm;
  G4double Pos_y1 = 0*mm;
  G4double Pos_z1 = 0*mm;

  G4RotationMatrix* uRot = new G4RotationMatrix;
  uRot->rotateZ(0.*rad);
  G4ThreeVector uTrans1(Pos_x1,Pos_y1,Pos_z1);
  G4SubtractionSolid* head_1 = new G4SubtractionSolid("head_1", head_Con, head_Trd, uRot, uTrans1);
//----------------------- head_2

  G4double Pos_x2 = -200.*mm;
  G4double Pos_y2 = 0*mm;
  G4double Pos_z2 = 0*mm;


  G4ThreeVector uTrans2(Pos_x2,Pos_y2,Pos_z2);
  G4SubtractionSolid* head_2 = new G4SubtractionSolid("head_2", head_1, head_Trd, 0, uTrans2);
//----------------------- head_3

  G4double Pos_x3 = 0.*mm;
  G4double Pos_y3 = 200*mm;
  G4double Pos_z3 = 0*mm;

  G4ThreeVector uTrans3(Pos_x3,Pos_y3,Pos_z3);
  G4SubtractionSolid* head_3 = new G4SubtractionSolid("head_3", head_2, head_Trd, 0, uTrans3);
//----------------------- head_4

  G4double Pos_x4 = 0.*mm;
  G4double Pos_y4 = -200*mm;
  G4double Pos_z4 = 0*mm;

  G4ThreeVector uTrans4(Pos_x4,Pos_y4,Pos_z4);
  G4SubtractionSolid* head = new G4SubtractionSolid("head", head_3, head_Trd, 0, uTrans4);
//------------------------------ end

  G4double  pDcz = 32.5 *mm;
...
//-------------------------down box
  G4double pdbz=0*mm;
 /* G4Box* down_box = new G4Box("down_box",dx1+0.0001*mm,dy1+0.0001*mm,pdbz+0.0001*mm);

 //----------------------------he + down=guide

  G4double aPos_x = 0. *mm;
  G4double aPos_y = 0. *mm;
  G4double aPos_z =-(pDConz+pdbz);

  G4RotationMatrix* aRot = new G4RotationMatrix;
  aRot->rotateZ(0.*rad);
  G4ThreeVector aTrans(aPos_x,aPos_y,aPos_z);
  G4UnionSolid* guide= new G4UnionSolid("guide", he, down_box, aRot, aTrans);
*/
//----------------------------scintillator_middle
  G4double  scintz = 5.0*mm;
  G4Box* scint_middle = new G4Box("scint_middle",dx1+0.0001*mm,dy1+0.0001*mm,0.0002*mm+scintz);

//--------------------------scintillator_middle+all=scintillator_up

  G4double suPos_x = 0. *mm;
  G4double suPos_y = 0. *mm;
  G4double suPos_z = pDConz+pdbz+scintz+0.0001*mm;

  G4RotationMatrix* suRot = new G4RotationMatrix;
  suRot->rotateX(0.*deg);
  G4ThreeVector suTrans(suPos_x,suPos_y,suPos_z);
  G4UnionSolid* scint_up= new G4UnionSolid("scint_up", scint_middle, guide, suRot, suTrans);
...
//--------------------------scintillator_up+guide=scint
  ...
  G4UnionSolid* scint_union= new G4UnionSolid("scint_union", scint_up, guide, suRot, suTrans);
  ...
  G4double guidePos_z = pDConz+pdbz+scintz;
 ...
 /////////////////////////////////////////////////////////////
   //****************** Build PMTs
    ...
    G4ThreeVector  phpos=G4ThreeVector(0.*mm,0.0*mm,pDConz+2*pDcz-2.5*mm);
